  ...
  try {
    ...
    if (Date.now() - Number(reportNow?.creationDate) >= timeDelay) {
      ...
      if (!alreadyblocked) {
        ...
        const ublockedsubgreddiits = userreportingto?.blockedSubgreddiits.concat(
          {
...
router.put("/api/deletecommentsfromsubgreddiit", authenticate, async (req, res) => {
...
router.put("/api/creatingposts", authenticate, async (req, res) => {
  ...
  try {
    ...
    const bannedKeyWords = subgreddiit?.bannedKeyWords?.map((obj) => obj.keyword);
